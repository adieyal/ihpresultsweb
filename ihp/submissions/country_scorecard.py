...
def get_country_indicators(country, questions, agencies_data):
    ...
    try:
        baseline_denom = questions["19"]["baseline_value"] / none_num(10000.0)
        latest_denom = questions["19"]["latest_value"] / none_num(10000.0)
    except TypeError:
        import traceback
        traceback.print_exc()

    # Outpatient Visits
    other_indicators["outpatient_visits_baseline"] = questions["20"]["baseline_value"] / baseline_denom
    other_indicators["outpatient_visits_latest"] = questions["20"]["latest_value"] / latest_denom
    other_indicators["outpatient_visits_change"], other_indicators["outpatient_visits_change_dir"] = calc_change(other_indicators["outpatient_visits_latest"], other_indicators["outpatient_visits_baseline"])

    # Skilled Personnel
    other_indicators["skilled_personnel_baseline"] = questions["18"]["baseline_value"] / baseline_denom
    other_indicators["skilled_personnel_latest"] = questions["18"]["latest_value"] / latest_denom
    other_indicators["skilled_personnel_change"], other_indicators["skilled_personnel_change_dir"] = calc_change(other_indicators["skilled_personnel_latest"], other_indicators["skilled_personnel_baseline"])

    # Health Workforce
    other_indicators["health_workforce_perc_of_budget_baseline"] = safe_div(questions["21"]["baseline_value"], none_num(questions["7"]["baseline_value"]))
    other_indicators["health_workforce_perc_of_budget_latest"] = safe_div(questions["21"]["latest_value"], questions["7"]["latest_value"])
    other_indicators["health_workforce_spent_change"], other_indicators["health_workforce_spent_change_dir"] = calc_change(questions["21"]["latest_value"], questions["21"]["baseline_value"])
...
#            if "4" in aqs and aqs["4"]["comments"]:
