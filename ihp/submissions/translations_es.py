...
agency_commentary_text = {
    "1DP" : "An IHP+ Country Compact or equivalent has been signed by the agency in %(cur_val).0f%% of IHP+ countries where they exist. Target = %(target_val).0f%%.",
    "2DPa" : u"In %(cur_year)s %(one_minus_cur_val).0f%% of health sector aid was reported by the agency on national health sector budgets - %(one_minus_diff_direction)s from %(one_minus_base_val).0f%%. Target = %(target_val).0f%% reduction in aid not on budget (with ≥ 85%% on budget).",
    "2DPb" : Template("In {{ cur_year }} {{ cur_val|floatformat }}% of capacity development was provided by the agency through coordinated programmes {% if diff_direction %}- {{ diff_direction }} from {{ base_val|floatformat }}%.{% endif %} Target = {{ target_val|floatformat }}%."),
    "2DPc" : "In %(cur_year)s %(cur_val).0f%% of health sector aid was provided by the agency through programme based approaches - %(diff_direction)s from %(base_val).0f%%. Target = %(target_val).0f%%.",
    "3DP" : "In %(cur_year)s %(cur_val).0f%% of health sector aid was provided by the agency through multi-year commitments - %(diff_direction)s from %(base_val).0f%%. Target = %(target_val).0f%%.",
    "4DP" : "In %(cur_year)s %(cur_val).0f%% of health sector aid disbursements provided by the agency were released according to agreed schedules - %(diff_direction)s from %(base_val).0f%% in %(base_year)s. Target = %(target_val).0f%%.",
    "5DPa" : Template("In {{ cur_year }} {{ one_minus_cur_val|floatformat }}% of health sector aid provided by the agency used country procurement systems{% if one_minus_diff_direction %} - {{ one_minus_diff_direction }} from {{ one_minus_base_val|floatformat }}%{% endif %}. Target = {{ target_val|floatformat }}% reduction in aid not using procurement systems (with ≥ 80% using country systems)."),
    "5DPb" : u"In %(cur_year)s %(one_minus_cur_val).0f%% of health sector aid provided by the agency used national public financial management systems - %(one_minus_diff_direction)s from %(one_minus_base_val).0f%%. Target = %(target_val).0f%% reduction in aid not using PFM systems (with ≥ 80%% using country systems).",
    "5DPc" : "In %(cur_year)s the stock of parallel project implementation units (PIUs) used by the agency in the surveyed countries was %(cur_val)s - %(diff_direction)s from %(base_val)s. Target = %(target_val).0f%% reduction in stock of PIUs.",
    "6DP" : "In %(cur_year)s national performance assessment frameworks were routinely used by the agency to assess progress in %(cur_val).0f%% of IHP+ countries where they exist. Target = %(target_val).0f%%.",
    "7DP" : "In %(cur_year)s the agency participated in health sector mutual assessments of progress in %(cur_val).0f%% of IHP+ countries where they exist. Target = %(target_val).0f%%.",
    "8DP" : "In %(cur_year)s, evidence exists in %(cur_val).0f%% of IHP+ countries that the agency supported civil society engagement in health sector policy processes. Target = %(target_val).0f%%.",
}
